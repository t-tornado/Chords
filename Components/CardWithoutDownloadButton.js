...
export default function CardWithoutDownloadButton({id, title, composer, artist, artWork, genre, duration }) {
...
// console.log('INFO---*CardWithoutDownloadButton*:::item from plalist is   =>',item)
...
const toPlay =async () => {
  ...
  openStoreMax()
  skipOffline(id,playerLoadedState,downloadsLoadedState)
  ...
  return (
    <TouchableOpacity style={card} activeOpacity={1} onPress={toPlay}  >
      <View style={cardCover}>
        ...
        source={artWork == undefined ?require('../assets/alternate_image.jpg') : {uri:artWork} }
        // source={{uri:artWork}}
      ...
      numberOfLines={1}
      ellipsizeMode='tail'
       style={{ color: Colors.download_card_title }} >{title}</Text>
        <Text 
        numberOfLines={1}
        ellipsizeMode='tail'
        style={{ 
        ...
        numberOfLines={1}
        ellipsizeMode='tail'
        style={{ color: Colors.download_card_choir }}>{artist}</Text>
      </View>
