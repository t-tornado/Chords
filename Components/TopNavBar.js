    ...
    <View style={styles.container}>
      <ScrollView ref={(list) => (scrollViewRef.current = list)} horizontal>
        {routes.map((route, index) => {
          ...
          let fontColor = {};
          isFocused
            ? (focusStyle = { backgroundColor: Colors.active_top_tab })
            : (focusStyle = { backgroundColor: Colors.non_active_top_tab });
          isFocused
            ? (fontColor = { color: Colors.active_top_tab_text })
            : (fontColor = { color: Colors.non_active_top_text });
