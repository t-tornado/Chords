...
import {downloadSong,
  ...
  onlineLoadFromDir,
  updateStoreOnlineLib} from '../Redux/actions/DownloadAction'
import {deleteSong,
  openDelete,
  closeDelete, 
  deleteFileSuccess, 
  afterDeleteSuccess} from '../Redux/actions/DeleteAction'
...
import {Songs, Offline} from '../Song'
import { fifteen, thirteen } from '../Config/Dimensions'
...
const [play, setPlay] = useState(false)
    ...
    return(
        ...
        <DownloadedSongsContext.Provider value={props.downloadedSongs} >
        <LoadFromDirContext.Provider value={props.loadFromDir} >
        <LoadPlayerActionContext.Provider value={props.loadPlayer} >
        <LoadPlayerStateContext.Provider value={props.playerLoaded} >
        <OpenDeleteStateContext.Provider value={props.openDeleteState}  >
        <DeleteSongActionContext.Provider value={props.deleteSongAction}  >
        <SongDeletedStateContext.Provider value={props.songDeletedState} >
        <openDeletContext.Provider value={props.openDeleteAction} >
        <closeDeleteContext.Provider value={props.closeDeleteAction} >
        <FileToDeleteContext.Provider value={props.filetoDelete} >
        <DownloadingSongSuccessContext.Provider value={props.downloadingSuccessState} >
        <afterDownloadSuccessAcion.Provider value={props.afterDownloadSuccess} >
        <afterDeleteSuccessAction.Provider value={props.afterDeleteSuccess}>
        <getDownloadProgressContext.Provider value={props.downloadProgress} >
        <DownloadingSongState.Provider value={props.downloadingSongState} >
          {/*SONG OPTIONS */}
        <GetLikeSongStateContext.Provider value={props.likeSongState} > 
        <GetUnlikeSongStateContext.Provider value={props.unlikeSongState} >
        <DownloadsLoadedContext.Provider value={props.getDownlaodsLoaded} >
        <WebLoadedContext.Provider value={props.getWebLoaded} >
        <dispatchDownloadsLoaded.Provider  value={props.downloadsLoaded} >
        <dispatchWebSongsLoaded.Provider value={props.webSongsLoaded } >
        <openMaxPlayer.Provider value={props.openMax} >
        <StoreMaxState.Provider value={props.storeMaxState} >
        <StorePlayerModeContext.Provider value={props.playerMode} >
        <dispatchPlayerOnline.Provider value={props.playerOnline} >
        <dispatchPlayerOffline.Provider value={props.playerOffline}>
        <likeSongActionsContext.Provider value={props.likeSong} >
        <QueueStateContext.Provider value={props.queueState} >
        <queueActionContext.Provider value={props.queueAction} >
        <CurrentQueueContext.Provider value={props.storeQueue } >
        <onlineLoadFromDirContext.Provider value={props.loadDirToOnline} >
        <updateOnlineLibCOntext.Provider value={props.updateOnlineLib} >
        <StoreOnlineLibContext.Provider value={props.storeOnlineLib} >
        <SongsFetchedStatesContext.Provider value={props.fetchedStates} >
        <FetchedSongsContext.Provider  value={props.fetchedSongs}>
        <RefreshPlayerActionContext.Provider value={props.refreshPlayer} >
        <ResetWebAndDownloadsLoadedContext.Provider value={props.resetLoadedStates} >
                      {props.children}
...
export const useOpenQueue = () => {
  ...
  const Func = React.useCallback((state)=> {
...
export const useSetPlayerQueue = () => {
  const dispatch= useContext(queueActionContext)
  const Func = React.useCallback(async (lstate)=> {
...
export const useSkipInQueue = () => {
  const Func = React.useCallback(async id => {
    await TrackPlayer.skip(id).then(()=> {
      updatePlayerOptions()
    }).catch(() => {
      null
    })
...
let arr = []
  const skipFunction = React.useCallback(async (id,playerLoadedState,downloadsLoadedState)=> {
    if(!playerLoadedState && !downloadsLoadedState){
          TrackPlayer.add([...offlineSongs]).then(async ()=> {
            downloadsLoaded()
              loadplayer()
          await TrackPlayer.skip(id).then(async ()=> {
    ...
    if( playerLoadedState && downloadsLoadedState) {
      ...
      if(!arr.includes(id)){
        offlineSongs.forEach(offline => {
          if(offline.id == id)
          TrackPlayer.add([offline]).then(async () => {
            await TrackPlayer.skip(id).then(async ()=> {
              await TrackPlayer.play().then(()=> {
                playerStore.play()
            updatePlayerOptions()
...
const skipFunction = React.useCallback(async (
  state , 
  id,
  anthemsFetchedState,
  hymnsFetchedState,
  ...
  if(!state && !webLoadedState){
  TrackPlayer.add([...webSongs]).then(async (tracks)=>{
    webLoadedDispatch()
    loadPlayer()
    await TrackPlayer.skip(id).then(async()=> {
      await  TrackPlayer.play().then(()=>null).catch(()=> null)
      playerStore.play()
      updatePlayerOptions()
}).catch(()=> null)
  }).catch(()=> null)
  ...
  if(state && !webLoadedState){
    TrackPlayer.add([...webSongs]).then(async ()=>{
      webLoadedDispatch()
      loadPlayer()
      await TrackPlayer.skip(id).then(async()=> {
        await  TrackPlayer.play().then(()=>null).catch(() => null)
  ...
  choralBluesFetchedState
    ...
    TrackPlayer.add([...fetchedAnthems]).then(async ()=>{
      webLoadedDispatch()
      loadPlayer()
      await TrackPlayer.skip(id).then(async()=> {
        await  TrackPlayer.play().then(()=>null).catch(() => null)
    ...
    choralBluesFetchedState
      ...
      TrackPlayer.add([...fetchedHymns]).then(async ()=>{
        webLoadedDispatch()
        loadPlayer()
        await TrackPlayer.skip(id).then(async()=> {
          await  TrackPlayer.play().then(()=>null).catch(() => null)
      ...
      choralBluesFetchedState
        ...
        TrackPlayer.add([...fetchedClassicals]).then(async ()=>{
          webLoadedDispatch()
          loadPlayer()
          await TrackPlayer.skip(id).then(async()=> {
            await  TrackPlayer.play().then(()=>null).catch(() => null)
        ...
        choralBluesFetchedState
          ...
          TrackPlayer.add([...fetchedKelencha]).then(async ()=>{
            webLoadedDispatch()
            loadPlayer()
            await TrackPlayer.skip(id).then(async()=> {
              await  TrackPlayer.play().then(()=>null).catch(() => null)
          ...
          choralBluesFetchedState
            ...
            TrackPlayer.add([...fetchedChristmasAnthems]).then(async ()=>{
              webLoadedDispatch()
              loadPlayer()
              await TrackPlayer.skip(id).then(async()=> {
                await  TrackPlayer.play().then(()=>null).catch(() => null)
            ...
            choralBluesFetchedState
              ...
              TrackPlayer.add([...fetchedEasterAnthems]).then(async ()=>{
                webLoadedDispatch()
                loadPlayer()
                await TrackPlayer.skip(id).then(async()=> {
                  await  TrackPlayer.play().then(()=>null).catch(() => null)
    ...
    !choralBluesFetchedState
      ...
      TrackPlayer.add([...fetchedChoralBlues]).then(async ()=>{
        webLoadedDispatch()
        loadPlayer()
        await TrackPlayer.skip(id).then(async()=> {
          await  TrackPlayer.play().then(()=>null).catch(() => null)
...
export const useDownloadSong = () =>{
  ...
  const toDownload = React.useCallback((title,id,url,artist,numDownloads,genre,artwork,duration,composer)=> {


  RNFS.readDir(downloadsPath).then(res => {
    if(res.length == 0){
  downloadAction(title,id,url,artist,numDownloads,genre,artwork,duration,composer)
  return
} else {
  res.forEach(file => {
  ...
  downloadAction(title,id,url,artist,numDownloads,genre,artwork,duration,composer)
  return
    } else if(file.name ==`Aa${title}Aa${artist}Aa${id}Aa${genre}Aa${duration}Aa${composer}Aa.mp3`) {
                return 
   }
  })
}}) 
...
const delFunc = React.useCallback(()=> {
  deleteAction(FileToDel.title,FileToDel.id,FileToDel.artist,FileToDel.genre, FileToDel.duration,FileToDel.composer)
...
const openFunc = React.useCallback((title,id,artist,genre,duration,composer,likes,artwork)=> {
  action(title,id,artist, genre, duration,composer,artwork,likes,artwork)
})
return openFunc
}
...
export const useLikeSong = () => {
  ...
  const Func = React.useCallback(async (songState,genre,id)=> {
      ...
      try {
        db.collection(genre).doc(`${id}`).get().then(doc => {
...
export const useTrackPlay = () => {
    ...
    const playSong =React.useCallback((lpState)=>{
         try {
...
export const useTrackNext = () => {
    ...
    const forward = React.useCallback(async ()=> {
          TrackPlayer.skipToNext().then(async ()=>{
...
export const useSetKeyword = () => {
  ...
  const setWord = React.useCallback((word)=> {
      value(word)
      return
  },[value])
  return setWord
...
const loadTracks = React.useCallback((source,lpState)=>{
     ...
     if(source=='download' && !lpState){
      ...
      if(Songs.length != 0) {
        TrackPlayer.add(Songs).then(res => {
          loadPlayer()
...
const mapDispatchToProps = (dispatch) => {
    return {
       ...
       downloadSongAction:(title,id,url,artist, numDownloads,genre,artwork,duration,composer)=> dispatch(downloadSong(title,id,url,artist,numDownloads,genre,artwork,duration,composer)),
       loadFromDir: ()=> dispatch(initialLoad()),
       loadPlayer: ()=> dispatch(loadPlayer()),
       refreshPlayer: () => dispatch(refreshPlayer()),
       deleteSongAction : (title,id,artist,genre,duration,composer) => dispatch(deleteSong(title,id,artist, genre,duration,composer)),
       openDeleteAction: (title,id,artist,genre,duration,composer,likes,artwork) => dispatch(openDelete(title,id,artist, genre,duration,composer,likes,artwork)),
...
export default connect(mapStateToProps, mapDispatchToProps)(TrackContextProvider)
...
setTim(Time)
  return () => {
    setTim(0)
      ...
      return (
      <Text style={{ color: "#e6e7e8" , fontSize: 17 }}>{tim} </Text>
      )
    }
