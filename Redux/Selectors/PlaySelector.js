...
export const getAllSongs = createSelector(storeSongs,(state)=>{
   ...
   let allSongs = anthems.concat(kelencha,classicals,hymns,easterAnthems,christmasAnthems,choralBlues)
...
export const getSearchedSongs = (state, keyword) => {
   ...
   const newSongs = songs.filter((song)=> song.title.toLowerCase().includes(keyword.toLowerCase()))
   return newSongs 
}
...
export const getOpenDeleteState = createSelector(openDeleteSelector, openDel => openDel)
