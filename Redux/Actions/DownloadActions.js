...
export const downloadSongSuccess = (
  title,
  id,
  artist,
  genre,
  ...
  return {
    ...
    payload: {
      ...
      artist: artist,
      url: artwork
        ? `file://${downloadSongsPath}/Aadd${title}Aa${artist}Aa10000000000${id}Aa${genre}Aa${duration}Aa${composer}Aa.mp3`
        : `file://${downloadSongsPath}/Aa${title}Aa${artist}Aa10000000000${id}Aa${genre}Aa${duration}Aa${composer}Aa.mp3`,
      artwork: artwork
        ? `file://${artworkDownloadPath}/Aa${title}Aa${artist}Aa10000000000${id}Aa.jpg`
        : null,
      duration: duration,
      genre: genre,
      composer: composer,
...
export const createMusicDirectory = () => {
  RNFS.mkdir(downloadSongsPath)
    .then(() => {})
    .catch(() => {
      null;
    });
  RNFS.mkdir(artworkDownloadPath)
    .then(() => {
      return;
    })
    .catch(() => {
...
export const downloadSong = (
  ...
  return (dispatch) => {
    ...
    RNFS.downloadFile({
      fromUrl: artwork,
      toFile:
        artworkDownloadPath + `/Aa${title}Aa${artist}Aa10000000000${id}Aa.jpg`,
    })
      .promise.then(() => {
        // DOWNLOAD SONG
        RNFS.downloadFile({
          ...
          .promise.then((res) => {
            dispatch(
              downloadSongSuccess(title, id, artist, genre, duration, composer)
            );
            try {
              db.collection(genre)
                .doc(`${id}`)
...
export const checkDir = () => {
  RNFS.readDir(downloadSongsPath)
    .then((res) => {
      res.forEach((item) => {
...
export const deleteDir = () => {
  RNFS.unlink(downloadSongsPath)
    .then((res) => {})
    .catch(() => null);
  RNFS.unlink(artworkDownloadPath)
    .then((res) => {})
    .catch(() => null);
  return;
};
    ...
    RNFS.readDir(downloadSongsPath)
      .then((res) => {
        res.forEach((file, index) => {
          ...
          const obj = {
            id: splitTitle[3],
            title: splitTitle[1].startsWith("dd")
              ? splitTitle[1].substring(2)
              : splitTitle[1],
            artist: splitTitle[2],
            url: splitTitle[1].startsWith("dd")
              ? `file://${downloadSongsPath}/Aadd${splitTitle[1]}Aa${splitTitle[2]}Aa${splitTitle[3]}Aa${splitTitle[4]}Aa${splitTitle[5]}Aa${splitTitle[6]}Aa.mp3`
              : `file://${downloadSongsPath}/Aa${splitTitle[1]}Aa${splitTitle[2]}Aa${splitTitle[3]}Aa${splitTitle[4]}Aa${splitTitle[5]}Aa${splitTitle[6]}Aa.mp3`,
            artwork: splitTitle[1].startsWith("dd")
              ? `file://${artworkDownloadPath}/Aa${splitTitle[1].substring(
                  2
                )}Aa${splitTitle[2]}Aa${splitTitle[3]}Aa.jpg`
              : null,
